...
class Calculo:
    ...
    def tiempoDeTrabajo(self, inicioDeServicio, finDeServicio):
        ...
        if dayEnd==dayStart:
            print("PASO2",  minuteStart, minuteEnd,  minuteEnd-15)
            """si hay 20 minutos de diferencia pero en horas distintas mismo dia. """
            if hourStart!=hourEnd 
            if hourStart==hourEnd and minuteEnd-15>minuteStart and weekDayStart<6:
                print("PASOOO start 45  end 05")
                """Si solo dura entre 15 a 59 minutos en dia de semana"""
                pagar.append(1)
                pagar.append(0) 
                return pagar
            if hourStart==hourEnd and minuteEnd-15>minuteStart  and weekDayStart>5:
...
Calcular=Calculo()
""" Caso1:  El mismo dia, y solo 25minutos"""
print(" El mismo dia, y solo 25minutos")
